...
public class Receta {
	...
	//Tener en cuenta que el precio de una receta se saca sumando el precio total de los ingredientes y las caracteristicas de la receta
	...
	//Calcula el precio total de las caracteristicas
	public double calcularPrecioCaracteristicas(String complejidadActual, String tipoDeRecetaActual) {
		...
		case "ensalada" -> 1000;
		case "postre" -> 1500;
		default -> throw new IllegalArgumentException("Valor no valido: " + tipoDeRecetaActual);
		};
	...
	public double calcularPrecioReceta() {
		...
		if(!this.recetasContenidas.isEmpty()) {
			for(int i = 0; i < this.recetasContenidas.size();i++) {
				...
				auxiliar += calcularPrecioCaracteristicas(this.recetasContenidas.get(i).getComplejidad(), this.recetasContenidas.get(i).getTipoDeReceta());
			}
			
		}else{
			//En caso de que la lista de recetas este vacia, solo se calcula el precio de esta receta
			...
			auxiliar += calcularPrecioCaracteristicas(this.complejidad, this.getTipoDeReceta());
		}
		return auxiliar;
	}
